...
Around Dispersion(double[], int, double, double, double);	//Dispersion relation for turning ImG_12 into ReG_12
...
Around Dispersion(double Par[], int Temp, double k0, double k, double theta)
{
...
Around Dispersion(double Par[], int Temp, double k0, double k, double theta, double a, double b, Around ImG12, int order, int deep)
{
...
Around k0_Int(double Par[], int Temp, double k, double theta)
{
	double a, b;	//Sub-interval limits of integration
...
void Characterize_k0_Int(double Par[], int Temp, double k, double theta, double zero[10], double gamma[10], int &Poles)
{
	...
	if(true)//Temp != 0)
		Lower = -sqrt(Par[4]+pow(Par[3], 2))/2.;	//Integrate from -E/2 to E/2
	else
...
#elif VERSION == 42
	zero[0] = .5*sqrt(complex<double>(4.*pow(k, 2), pow(Par[1], 2))).real();	//Potential poles, I know exactly where these are at.
	zero[1] = -.5*sqrt(complex<double>(4.*pow(k, 2), pow(Par[1], 2))).real();
	gamma[0] = abs(.5*sqrt(complex<double>(4.*pow(k, 2), pow(Par[1], 2))).imag());
	gamma[1] = abs(-.5*sqrt(complex<double>(4.*pow(k, 2), pow(Par[1], 2))).imag());
#endif

	zero[2] = .5*(sqrt(Par[4]+pow(Par[3], 2))-real(sqrt(complex<double>(4.*(pow(k, 2)+pow(Par[2], 2)-k*Par[3]*cos(theta))+pow(Par[3], 2)-2.*pow(GAMMA, 2), 2.*sqrt(4.*pow(Par[2]*GAMMA, 2)-pow(GAMMA, 4))))));	//Exact vacuum poles
	zero[3] = .5*(sqrt(Par[4]+pow(Par[3], 2))+real(sqrt(complex<double>(4.*(pow(k, 2)+pow(Par[2], 2)-k*Par[3]*cos(theta))+pow(Par[3], 2)-2.*pow(GAMMA, 2), 2.*sqrt(4.*pow(Par[2]*GAMMA, 2)-pow(GAMMA, 4))))));
	zero[4] = .5*(-sqrt(Par[4]+pow(Par[3], 2))-real(sqrt(complex<double>(4.*(pow(k, 2)+pow(Par[2], 2)+k*Par[3]*cos(theta))+pow(Par[3], 2)-2.*pow(GAMMA, 2), 2.*sqrt(4.*pow(Par[2]*GAMMA, 2)-pow(GAMMA, 4))))));
	zero[5] = .5*(-sqrt(Par[4]+pow(Par[3], 2))+real(sqrt(complex<double>(4.*(pow(k, 2)+pow(Par[2], 2)+k*Par[3]*cos(theta))+pow(Par[3], 2)-2.*pow(GAMMA, 2), 2.*sqrt(4.*pow(Par[2]*GAMMA, 2)-pow(GAMMA, 4))))));
	...
	{
		gamma[2] = gamma[3] = abs(.5*imag(sqrt(complex<double>(4.*(pow(k, 2)+pow(Par[2], 2)-k*Par[3]*cos(theta))+pow(Par[3], 2)-2.*pow(GAMMA, 2), 2.*sqrt(4.*pow(Par[2]*GAMMA, 2)-pow(GAMMA, 4))))));
		gamma[4] = gamma[5] = abs(.5*imag(sqrt(complex<double>(4.*(pow(k, 2)+pow(Par[2], 2)+k*Par[3]*cos(theta))+pow(Par[3], 2)-2.*pow(GAMMA, 2), 2.*sqrt(4.*pow(Par[2]*GAMMA, 2)-pow(GAMMA, 4))))));
	}
...
double Newton_Method_k0(double k0, double Par[], double k, double theta, int Temp, double (*Folding)(double[], double, double, double, int))	//Newton's method for finding poles of f by looking for zeros of 1/f, much more stable to the point of absolute confidence
{
	...
	double Exit;
	int i = 0;
	double Danger[] = {.5*(sqrt(Par[4]+pow(Par[3], 2))+real(sqrt(complex<double>(4.*(pow(k, 2)+pow(Par[2], 2)-k*Par[3]*cos(theta))+pow(Par[3], 2)-2.*pow(GAMMA, 2), 2.*sqrt(4.*pow(Par[2]*GAMMA, 2)-pow(GAMMA, 4)))))), .5*(-sqrt(Par[4]+pow(Par[3], 2))-real(sqrt(complex<double>(4.*(pow(k, 2)+pow(Par[2], 2)+k*Par[3]*cos(theta))+pow(Par[3], 2)-2.*pow(GAMMA, 2), 2.*sqrt(4.*pow(Par[2]*GAMMA, 2)-pow(GAMMA, 4))))))};
...
__host__ double Host::Imk0_Integrand(double Par[], double k0, double k, double theta, int Temp)	//Integrand of the folding integral for positive energy
{
	...
	double omega[2] = {sqrt(Par[4]+pow(Par[3], 2))/2.+k0, sqrt(Par[4]+pow(Par[3], 2))/2.-k0};
