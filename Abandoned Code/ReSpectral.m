...
If[Master==$Failed,Exit[]]; (*Did the file import, if not Exit as the rest will be painful to complete*)
dim=Dimensions[Master][[1]];
parameters = Select[Master, Dimensions[#][[1]] == 5 &]; (*Collect the parameters*)
Master = Select[Master, Dimensions[#][[1]] != 5 &];	(*Remove the parameters from the rest of the data*)
upper =Master[[-1,1]]; (*upper edge of the s>=0 table*)

(*Functions to convert calls to s and P in the negative s region to i and j*)
i1[s_,P_]:=10Sqrt[s+P^2]
j1[s_,P_]:=10(P-Sqrt[s+P^2])
i2[s_,P_]:=936/5+Sqrt[s+P^2]
j2[s_,P_]:=10(P-Sqrt[s+P^2])
(*Functions to convert calls to any i and j to s and P*)
...
(*Dispersion relation for any f and i and j. There's some funny business around knots of the interpolation. I don't know what the difference is between these two.*)
...
(*Sort the data into tables according to the which conversion between i and j to s and P applies*)
...
(*Force all numbers to machine real numbers. I think this will force certain subnormal numbers to zero.*)
If[Temp != "0",
...
(*Creat the interpolations and how each region connects to the next. Sometimes the above code doesn't make all numbers machine real, so error INterpolation::mspl is silenced*)
...
(*Silence the error messages for 'function is zero everywhere', 'integration failed to converge', 'convergence of integration is proceeding in the wrong direction', 'precision was lost', "integral didn't converge", "integral subdivided to zero interval", "division by zero in interval of analytic function"*)
...
(*Calculate the real contributions in the space-like mesons*)
...
(*Calculate the real contributions in the time-like mesons, which may include, 'do nothing'*)
