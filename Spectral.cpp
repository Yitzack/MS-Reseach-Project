...
int main(int argc, char* argv[])
{
	...
	long double Par[5] = {-26.796184939764153, 8.66343167061543, 1.8, 0, 0};
	Elements holder;
	...
	for(i = Start; i <= Finish; i++)	//Argv[6] allows to restart where ever
	{
		...
		for(j = iProcess; j < 616; j+=Total)	//Does the subset of E that has been assigned to this process, calculation loop
		{
			...
			ParPrivate[0] = -26.796184939764153*G*pow(pow(8.66343167061543,4)/(pow(8.66343167061543,4)+pow(g*T,4)),2);
			ParPrivate[1] = pow(pow(8.66343167061543,4)+pow(g*T,4),.25);
			ParPrivate[2] = Mq;
