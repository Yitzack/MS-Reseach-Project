...
int main(int argc, char* argv[])
{
	...
	for(i = atoi(argv[6]); i <= 0; i++)	//Argv[6] allows to restart where ever
	{
		#pragma omp parallel for
		for(j = iProcess+151; j < 616; j+=Total)	//Does the subset of E that has been assigned to this process, calculation loop
		{
	long double Par[5] = {-1.2236634430274063, 14.488216383738036, 1.8, 0, 0};
			Par[1] = 14.488216383738036;
			Par[2] = 1.8;
			if(argc == 4)
				switch(Temp)
				{
	...
	cout << "{" << flush;
		for(j = iProcess+151; j < 616; j+=Total)	//Does the subset of E that has been assigned to this process, output loop
		{
			long double Par[5] = {-0.16978114406756695, 8.59442426115969, 1.8, 0, 0};
			if(j <= 150)
			{
				if(i <= 208)
				{
