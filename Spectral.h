...
long double ImSelf_Energy(long double M, long double omega, long double k, int Temp)	//Single quark self energy
{
	...
	switch(Temp)
	{
		...
		case 1://194MeV
			...
			//omega0 = sqrt(pow(1.84184+Shift,2)+pow(k,2))+.248573;
		...
		case 2://285MeV
			...
			//omega0 = sqrt(pow(1.69584+Shift,2)+pow(k,2))+.247213;
		...
		case 3://320MeV
			...
			//omega0 = sqrt(pow(1.59439+Shift,2)+pow(k,2))+.258289;
		...
		case 4://400MeV
			...
			//omega0 = sqrt(pow(1.48038+Shift,2)+pow(k,2))+.253076;
...
{
	return(2.*ImSelf_Energy(M, omega, k, Temp)/(pow(pow(omega,2)-pow(k,2)-pow(M,2)-2.*M*ReSelf_Energy(M, omega, k, Temp),2)+pow(ImSelf_Energy(M, omega, k, Temp),2)));
}
